...
class ErlAstarCostmap2dOnline {
 public:
  ...
  explicit ErlAstarCostmap2dOnline(ros::NodeHandle &nh) : nh(nh) {
    ...
    costmap_sub = nh.subscribe<nav_msgs::OccupancyGrid>(costmap_topic, 1, &ErlAstarCostmap2dOnline::costmapCallback, this);
 ...
 private:
  ...
  void callAStar(double start_x, double start_y) {
    ...
    ROS_INFO("Finished Astar: Path 2D Length (in cell): %d\t\t Path Cost: %.2f\n", output.action_idx.size(), output.pcost);
    ...
    if (metric_path.size() > 0){
      ...
      auto lindix = this->planning_env->map->subv2ind(start_cell);
      int start_cost = this->planning_env->map->map().at(lindix);
      ROS_WARN_THROTTLE(1.0, "\nstart costmap val = %d", start_cost);

      lindix = this->planning_env->map->subv2ind(goal_cell);
      int goal_cost = this->planning_env->map->map().at(lindix);
      ROS_WARN_THROTTLE(1.0, "\ngoal costmap val = %d , %d (planning_cutoff_cost)", goal_cost, planning_cutoff_cost);

      // TODOZHL hot fix add start twice, to become a dummy valid path in downstream
      metric_path.push_back({start_x, start_y});
      metric_path.push_back({start_x, start_y});
    }
   ...
   * @param msg The incoming Odometry message.
   */
  void costmapCallback(const nav_msgs::OccupancyGrid::ConstPtr &gridmap_msg) {
    ...
    // ASTAR_COSTMAP2D do not use inflation here, put hard inflation in collision check
    // grid map is not binary
    auto gm = erl::fromROS(*gridmap_msg, true, false);
    ...
    if (!map_received) {
      ...
      curr_env.setPlanningObstacleLowerBound((uint8_t) planning_cutoff_cost);
