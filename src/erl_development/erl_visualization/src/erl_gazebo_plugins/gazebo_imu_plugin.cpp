...
void GazeboImuPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  getSdfParam<std::string>(_sdf, "imuTopic", imu_topic_, default_topics::IMU);
...
// #endif
  ...
  math::Vector3 W_vel = link_->GetWorldLinearVel();
  math::Vector3 W_a = (W_vel - velocity_prev_W_) / dt;
  math::Vector3 acceleration_I = C_W_I.RotateVectorReverse(W_a - gravity_W_);
  velocity_prev_W_ = W_vel;
