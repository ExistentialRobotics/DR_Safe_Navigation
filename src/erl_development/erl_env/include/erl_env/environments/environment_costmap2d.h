...
class Environment2D : public EnvironmentMetric<std::vector<int>, U> {
 public:
    ...
    for (unsigned tr = 0; tr < num_traj; ++tr)
      for (unsigned len = 0; len < len_traj; ++len) {
        ...
        for (unsigned int len_fine = 0; len_fine < len_fine_sz; ++len_fine) {
          ...
          // ASTAR_COSTMAP2D check traversalbity, map here is 1d
          auto cell_cost_xy = map->map().at(lindix);
          // JACKAL circumscribed radius round up 0.3 m, cost 77 (previous 0)
          // hence, planner will treat this as 0.3 inflation radius
          if ((unsigned int)cell_cost_xy > _planning_cutoff_cost) {
            collided = true;
            break;
          }
          ...
          if (len_fine == len_fine_sz - 1) {
            ...
            // ASTAR_COSTMAP2D calculate cost using nonbinary costmap (previous cell_cost_xy = 0)
   ...
   */
  void setPlanningObstacleLowerBound(uint8_t lb) {
    // set planning obstacle lb, greater than lb, will be considered obstacle in 
    // planner's view , ASTAR_COSTMAP2D
    _planning_cutoff_cost = lb;
  }

 private:

  // private member planning check obstacle lower bound ASTAR_COSTMAP2D
  uint8_t _planning_cutoff_cost = 0;
